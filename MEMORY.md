...
- **新闻采编资质**：没有，所以不能代运营、不能采写、不能加工稿件内容
- **服务形式**：提供新媒体设计及技术服务，只做来稿排版

### 系统操作规范（必须遵守）

- **重启 Gateway**：必须先征得小杨同意，未经同意不得自行重启
- **关闭/重启 Chrome**：必须先报告
- **删除文件**：必须先确认
- **发布操作**：必须先确认发布范围
- **每日日志**：
  - 新的一天小杨开始和我聊天时，立即创建 `memory/YYYY-MM-DD.md` 并开始记录
  - 重要内容、功能、技巧、踩坑、配置变更、规则更新必须及时写入相关文件
  - 不需要小杨提醒，自动执行
- **心跳检测**：
  - 每30分钟检测一次是否有未执行的任务
  - 检查是否有待处理的重要事项
